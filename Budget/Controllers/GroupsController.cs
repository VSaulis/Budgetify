...
namespace Budget.Controllers
{
    ...
    public class GroupsController : ControllerBase
    {
        ...
        public async Task<IActionResult> Add([FromBody] AddGroupRequest request)
        {
            ...
            var response = await _groupService.AddAsync(request);
        ...
        public async Task<IActionResult> List([FromQuery] ListGroupsRequest request)
        {
            ...
            var response = await _groupService.ListAsync(request);
            if (!response.IsValid) return BadRequest(response.Message);
            return Ok(response);
        }
        ...
        [HttpPut]
        [Authorize]
        public async Task<IActionResult> Edit([FromBody] EditGroupRequest request)
        {
            if (!ModelState.IsValid) return BadRequest(ModelState.GetErrorMessages());

            var response = await _groupService.EditAsync(request);
            if (!response.IsValid) return BadRequest(response.Message);
            return Ok(response);
        }
